...
async function createCheckoutToken({ checkoutOptions, creditCard }) {
  if (process.client) {
    const { sellerId, publishableKey } = checkoutOptions;
    ...
    const { response } = await requestCheckoutToken(params);
    return response.token.token;
  }
  return null;
}

async function requestCheckoutToken(params) {
  const mode =
    process.env.NODE_ENV !== "development" ? "production" : "sandbox";
