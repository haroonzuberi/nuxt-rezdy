...
export const getSessions = async ({ $axios, params }) => {
  ...
  const fetchSessions = offset => {
    return $axios.$get("/availability", { params: { ...options, offset, limit:1000000 } });
  };
...
export function getCompany({ $axios, companyAlias }) {
  return $axios.$get(`companies/alias/${companyAlias}`);
}
...
export function updateBookingStatus({ $axios, orderNumber, booking, status }) {
  return $axios.$put(`/bookings/booking/${orderNumber}`, { ...booking, status });
}
