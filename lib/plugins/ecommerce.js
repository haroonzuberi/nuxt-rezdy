...
export default ({ app, store }, inject) => {
    inject('ecommerce', {
      ...
      async trackCheckoutStep(options) {
        const { $gtm, $fb, $gtag } = app;
        const { step, option, item } = options;
          ...
          $gtm.pushEvent({
            ...
            ecommerce: {
              checkout: {
                ...
                products: await Promise.all([...(item ? [item] : []), ...items].map(async item => {
                  const product = await app.$rezdy.getProductByCode(item.productCode)
                  return {
                    id: item.productCode,
                    name: product.name,
        ...
        if ($gtag) {
            $gtag('event', step === 1 ? 'begin_checkout' : 'checkout_progress', {
                items:  await Promise.all([item, ...items].map(async item => {
                    const product = await app.$rezdy.getProductByCode(item.productCode)
      ...
      async trackPurchase(options) {
        ...
        if ($gtag) {
          ...
          $gtag('event', 'purchase', eventData2);
        }
        if ($fb) {
          $fb.track('Purchase', {
